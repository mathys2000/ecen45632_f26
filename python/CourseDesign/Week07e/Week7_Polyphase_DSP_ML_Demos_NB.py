...
# %% [markdown]
# # Week 7 — Polyphase Structures: DSP + ML Demos
#
# This notebook supports Lecture 13 and Lecture 14. The main idea is **compute only what survives the rate change**. Polyphase decomposition reorganizes FIR arithmetic so filtering occurs at the lowest practical rate, while filter banks extend the same decomposition into multiple subbands.
#
# The ML bridge is conceptual: grouped/depthwise convolutions and fixed subband front ends also partition computation into structured, efficient branches, but they partition different axes and are not mathematically identical to polyphase decomposition.
...
# %% [markdown]
# ## Lecture 13 — 1. Polyphase decomposition
# For decimation by $M$, split the FIR taps into $M$ phases: $e_k[n]=h[nM+k]$. Then $H(z)=\sum_{k=0}^{M-1} z^{-k}E_k(z^M)$.

# %%
demo_polyphase_decomposition()

# %% [markdown]
# ## 2. Efficient decimation
# The naive system computes all FIR outputs even though the downsampler retains only one out of every $M$. A polyphase implementation computes only the retained outputs.

# %%
demo_efficient_decimation()

# %% [markdown]
# ## 3. First Noble Identity
# A filter of the special form $G(z^M)$ may be commuted across a downsampler: $G(z^M) \rightarrow \downarrow M$ is equivalent to $\downarrow M \rightarrow G(z)$. This does **not** hold for an arbitrary $G(z)$.

# %%
demo_first_noble_identity()

# %% [markdown]
# ## 4. ML bridge — grouped convolution
# Grouped convolution partitions input/output **channels** into independent groups. Polyphase decomposition partitions FIR taps and input samples by **time phase modulo $M$**. The shared idea is structured parallelism, not exact equivalence.
...
# %% [markdown]
# ## 5. Second Noble Identity
# The interpolation counterpart is $\uparrow L \rightarrow G(z^L)$ equivalent to $G(z) \rightarrow \uparrow L$. Again, the special $z^L$ structure matters.

# %%
demo_second_noble_identity()

# %% [markdown]
# ## 6. Efficient interpolation
# Rather than multiply the many inserted zeros by FIR coefficients, polyphase interpolation computes each output phase directly from the low-rate input.

# %%
demo_efficient_interpolation()

# %% [markdown]
# ## 7. Two-channel Haar analysis/synthesis bank
# A critically sampled two-channel bank maps pairs of samples into lowpass-like averages and highpass-like differences. The orthonormal Haar pair reconstructs exactly and preserves energy.

# %%
demo_haar_filter_bank()

# %% [markdown]
# ## 8. Fixed filter-bank features + Scikit-Learn
# A non-trainable DSP front end can compress each waveform into a few subband energies. A small classifier then learns in this low-dimensional feature space.

# %%
demo_fixed_filterbank_ml()

# %% [markdown]
# ## 9. Tiny PyTorch classifier after a fixed DSP front end
# This final demo emphasizes an edge-deployment pattern: use inexpensive fixed features, then train only a very small classifier.
...
# 1. Change $M$ and verify how phase lengths change when the FIR length is not divisible by $M$.
# 2. Time the naive and `upfirdn` decimators on a long signal.
# 3. Break the first Noble Identity by replacing $G(z^M)$ with an arbitrary FIR and explain the mismatch.
# 4. Build a 4-channel Haar-like packet tree and inspect subband energies.
# 5. Compare fixed filter-bank features with a learned `Conv1d` front end using the same number of output features.
